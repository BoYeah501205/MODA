...
function BulkStatusPanel(props) {
    ...
    useEffect(function() {
        if (shiftDays.length > 0 && (!selectedDate || !shiftDays.some(function(d) { return d.date === selectedDate; }))) {
            setSelectedDate(shiftDays[0].date);
        }
    }, [shiftDays]);
    ...
    function handleExecute() {
        ...
        var statusLabels = { complete: 'Complete', wip: 'WIP', stopped: 'Stop', na: 'N/A' };
    ...
    if (!weekSchedule) {
        return <div className="p-3 text-xs text-gray-400 italic">No week schedule loaded</div>;
    }
    ...
    return (
        <div className="p-3 space-y-3 border-t border-gray-200 dark:border-gray-700">
            ...
            <div>
                <label className="block text-xs font-medium text-gray-600 dark:text-gray-300 mb-1">Shift</label>
                <div className="flex gap-2">
                    {['shift1', 'shift2'].map(function(s) {
                        var label = s === 'shift2' ? 'Shift 2' : 'Shift 1';
                        return <button key={s} onClick={function() { setSelectedShift(s); }}
            ...
            <div>
                ...
                <div className="flex gap-1 flex-wrap">
                    {shiftDays.map(function(day) {
                        return <button key={day.date} onClick={function() { setSelectedDate(day.date); }}
                        ...
                        >{day.label + ' ' + day.dayNum}</button>;
                    })}
                </div>
            </div>

            <div>
                <label className="block text-xs font-medium text-gray-600 dark:text-gray-300 mb-1">Status to apply</label>
                <select value={bulkStatus} onChange={function(e) { setBulkStatus(e.target.value); }}
                    className="w-full px-3 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-sm min-h-[44px]">
            ...
            <div>
                <label className="block text-xs font-medium text-gray-600 dark:text-gray-300 mb-1">Module scope</label>
                <div className="space-y-1">
                    <label className="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-200">
                        <input type="checkbox" checked={scopeScheduled} onChange={function(e) { setScopeScheduled(e.target.checked); }} /> Scheduled
                    </label>
                    <label className="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-200">
                        <input type="checkbox" checked={scopePrevious} onChange={function(e) { setScopePrevious(e.target.checked); }} /> Previous (trailing)
                    </label>
                    <label className="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-200">
                        <input type="checkbox" checked={scopeUpcoming} onChange={function(e) { setScopeUpcoming(e.target.checked); }} /> Upcoming
                    </label>
                </div>
            </div>

            <div>
                <label className="block text-xs font-medium text-gray-600 dark:text-gray-300 mb-1">Traveler sign-off</label>
                <select value={signTravelers ? 'sign' : 'leave'} onChange={function(e) { setSignTravelers(e.target.value === 'sign'); }}
                    className="w-full px-3 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-sm min-h-[44px]">
                    <option value="leave">Leave travelers untouched</option>
                    <option value="sign">Sign travelers (set to Signed)</option>
                </select>
            </div>
            ...
            >{running ? 'Running...' : 'Apply Bulk Status'}</button>

            {resultMsg && (
                <div className={'p-2 rounded-lg text-xs font-medium ' + (resultMsg.includes('Error') || resultMsg.includes('failed') ? 'bg-red-50 dark:bg-red-900/30 text-red-700 dark:text-red-300' : 'bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-300')}>
                    {resultMsg}
                </div>
            )}
        </div>
...
function AdminConfigTab(props) {
    ...
    function handleSaveStg(deptId) {
        ...
        setOrderedDepts(function(prev) {
            return prev.map(function(d) {
                if (d.id === deptId) {
                    ...
                    return Object.assign({}, d, { stagger_offset: parsed });
                }
                return d;
            });
        });
        ...
        var client = window.MODA_SUPABASE && window.MODA_SUPABASE.client ? window.MODA_SUPABASE.client : window.supabaseClient;
        if (client) {
            client.from('station_departments').update({ stagger_offset: parsed }).eq('id', deptId)
                .then(function(res) {
                    if (res.error) throw res.error;
                    setStgSaved(deptId);
                    setTimeout(function() { setStgSaved(null); }, 1500);
                ...
                .catch(function(err) {
                    ...
                    setOrderedDepts(function(prev) {
                        return prev.map(function(d) {
                            if (d.id === deptId) return Object.assign({}, d, { stagger_offset: oldVal });
                            return d;
                        });
                    });
                });
    ...
    return (
        <div ref={adminScrollRef} onScroll={function(e) { adminScrollPos.current = e.target.scrollTop; }} className="px-3 py-4 space-y-3 max-w-lg mx-auto overflow-y-auto" style={{ maxHeight: '100%' }}>
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: '3px', flexShrink: 0 }}>
                                        <span className="text-[10px] text-gray-400">stg:</span>
                                        {stgEditing === dept.id ? (
                                            <input
                                                type="number"
                                                min="0"
                                                step="1"
                                                value={stgValue}
                                                onChange={function(e) { setStgValue(e.target.value); }}
                                                onBlur={function() { handleSaveStg(dept.id); }}
                                                onKeyDown={function(e) { if (e.key === 'Enter') { e.target.blur(); } if (e.key === 'Escape') { setStgEditing(null); } }}
                                                autoFocus
                                                style={{ width: '42px', padding: '1px 4px', fontSize: '11px', borderRadius: '4px', border: '1px solid #93c5fd', textAlign: 'center', outline: 'none' }}
                                            />
                                        ) : (
                                            <button type="button" onClick={function() { setStgEditing(dept.id); setStgValue(String(dept.stagger_offset || 0)); }}
                                                style={{ minWidth: '24px', padding: '1px 4px', fontSize: '11px', borderRadius: '4px', border: '1px solid transparent', color: '#6b7280', cursor: 'pointer', background: 'transparent', textAlign: 'center' }}
                                                className="hover:border-gray-300 hover:bg-gray-100"
                                                title="Click to edit stagger offset">
                                                {dept.stagger_offset || 0}
                                            </button>
                                        )}
                                        {stgSaved === dept.id && <span className="text-[9px] text-green-600 font-medium">saved</span>}
                                    </span>
            ...
            {/* Bulk Status Tool (Catch-Up) */}
            <div className="border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden">
                <button onClick={function() { handleTogglePanel('bulkStatus'); }} className="w-full flex items-center justify-between px-4 py-3 bg-gray-50 dark:bg-gray-800 min-h-[48px]">
                    <span className="font-semibold text-sm text-gray-900 dark:text-white">Bulk Status Tool (Catch-Up)</span>
                    <span className="text-gray-400">{openPanel === 'bulkStatus' ? '\u25BC' : '\u25B6'}</span>
                </button>
                {openPanel === 'bulkStatus' && (
                    <BulkStatusPanel
                                            ...
                                            <td key={day.key + '-' + i} style={{ border: cellBorder, borderLeft: borderLeftStyle, background: s.cellBg, padding: '2px', textAlign: 'center', verticalAlign: 'middle' }}>
                                                <div className="module-tile" style={{
                                                    ...
                                                    {(function() {
                                                        ...
                                                        return (
                                                            <div style={{ display: 'flex', gap: '2px', justifyContent: 'center', flexWrap: 'wrap', marginTop: '2px', padding: '0 1px' }}>
                                                                {dots.map(function(k) {
                                                                    return <span key={k} style={{ width: '5px', height: '5px', borderRadius: '50%', background: DIFFICULTY_DOT_COLORS[k], flexShrink: 0, WebkitPrintColorAdjust: 'exact', printColorAdjust: 'exact' }} />;
                                                                })}
                                                            </div>
                                                        );
                                                    })()}
                ...
                {summaryView === 'daily-board' && (
                    ...
                    {(function() {
                        var activeDots = Object.keys(DIFFICULTY_DOT_COLORS).filter(function(k) {
                            return threeDayWindow.some(function(day) {
                                ...
                                for (var i = 0; i < depts.length; i++) {
                                    ...
                                    if (mods.some(function(m) { return m.difficulties && m.difficulties[k]; })) return true;
                                }
                                return false;
                            });
                        });
                        ...
                        var DIFFICULTY_LABELS = {
                            sidewall: 'Ext Sidewall', stair: 'Stair', hr3Wall: '3HR-Wall',
                            hr2Wall: '2HR-Wall', short: 'Short', doubleStudio: 'Dbl Studio',
                            common: 'Common Area', tile: 'Tile', sawbox: 'Sawbox'
                        };
                        return (
                            <div className="wsb-no-print" style={{ display: 'flex', flexWrap: 'wrap', gap: '10px', padding: '8px 12px', marginTop: '8px', background: '#f9fafb', borderRadius: '6px', border: '1px solid #e5e7eb' }}>
                                <span style={{ fontSize: '10px', fontWeight: 600, color: '#6b7280', marginRight: '4px', alignSelf: 'center' }}>Difficulty indicators:</span>
                                {activeDots.map(function(k) {
                                    return (
                                        <span key={k} style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '10px', color: '#374151' }}>
                                            <span style={{ width: '8px', height: '8px', borderRadius: '50%', background: DIFFICULTY_DOT_COLORS[k], flexShrink: 0 }} />
                                            {DIFFICULTY_LABELS[k]}
                                        </span>
                                    );
                                })}

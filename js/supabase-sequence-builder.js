 ...
 * @param {number} config.buildings - Number of buildings
 * @param {Array<{building: number, levels: number, modulesPerLevel: number}>} config.buildingConfigs
 * @param {string} config.serialPrefix - Prefix for serial numbers
 * @param {number} config.startingSerial - Starting serial number
 * @returns {Promise<Array>} Created modules
 */
    ...
    for (let level = 1; level <= levels; level++) {
      for (let moduleNum = 1; moduleNum <= modulesPerLevel; moduleNum++) {
        ...
        modules.push({
          ...
          blm_id: `B${building}L${level}M${String(moduleNum).padStart(2, '0')}`,
          building: building,
          level: level,
          unit_type: '',
